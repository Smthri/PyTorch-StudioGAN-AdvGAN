...
def load_sr_module(path, device, logger):
    from models.srgenerator import SRGenerator
    sr_module = SRGenerator(2)
    sr_module.to(device)
    sr_module.load_state_dict(torch.load(path, map_location=torch.device(device)))
    for param in sr_module.parameters():
        param.requires_grad=False
    sr_module.eval()
    if device == 0:
        logger.info(f'Loaded SR module from {path}')
    return sr_module


def load_target_model(path, device, logger):
    AdvDis = torch.hub.load('pytorch/vision:v0.10.0', 'resnet34', pretrained=False)
    AdvDis.to(device)
    AdvDis.load_state_dict(torch.load(path, map_location=torch.device(device)))
    for param in AdvDis.parameters():
        param.requires_grad=False
    AdvDis.eval()
    if device == 0:
        logger.info(f'Loaded target model as discriminator from {path}')
    return AdvDis
